...
async def check_and_remind_overdue_approvals(bot_instance: Bot):
    ...
    Включает список ID всех просроченных запросов, каждый на новой строке.
    """
    ...
    try:
        ...
        for manager_id, requests in manager_requests.items():
            try:
                ...
                # --- Формируем список ID запросов, каждый на новой строке ---
                # Используем \n для переноса строки. Telegram отобразит это корректно.
                # Оборачиваем в <code>, чтобы выглядело как код (моноширинный шрифт, удобно для копирования)
                request_id_list = '\n'.join([f"<code>{req['request_id']}</code>" for req in requests])
                ...
                notified_count += 1
                # Логируем только общее количество для краткости
                logging.info(f"🔔 Напоминание отправлено менеджеру {manager_id} о {count} запросах.")
                await asyncio.sleep(0.1)
            except Exception as e:
...
async def process_reason_input_for_top0(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Вы можете продолжить работу с ботом. После обработки запроса вы получите уведомление.",
        parse_mode='HTML',
        reply_markup=main_menu_keyboard(message.from_user.id)
    )
