...
def format_task_message(task_id: str, task: dict) -> str:
    ...
    - Не использует markdown_decoration.quote.
    """
    lines = []

    # 1. Заголовок с ID задачи (экранируем ID для безопасности)
    # Используем встроенный escape_markdown или аналог, если markdown_decoration.quote не хотим
    # aiogram 3.x имеет aiogram.utils.text_decorations.markdown_decoration
    # Но по требованию убираем. Для ID можно вручную экранировать базовые символы или оставить как есть
    # если ID гарантированно числовой. Для максимальной простоты - вставляем как есть.
    # Если ID может быть нечисловым и содержать _ * и т.п., то нужен какой-то escape.
    # Т.к. task_id приходит как str, и мы его не экранируем, предполагаем, что он безопасен
    # или пользователь знает, что делает. Для числовых ID из Sheets это верно.
    ...
    if creator:
        # Минимальное "экранирование" для полей, которые мы не контролируем полностью,
        # чтобы избежать базовых ошибок. Можно заменить на простую замену символов.
        # Для простоты оставим как есть, если уверены в данных.
        # Или сделаем простой escape для символов * _ ` [
        def simple_escape(text):
            # Экранируем только самые базовые символы Markdown, если нужно
            # Это компромисс между "никакого экранирования" и markdown_decoration.quote
            return text.replace('*', '\\*').replace('_', '\\_').replace('`', '\\`').replace('[', '\\[')
        
        escaped_creator = simple_escape(creator)
        lines.append(f"👤 Создал: {escaped_creator}")

    # 4. Дедлайн
    deadline = task.get('deadline', '')
    if deadline:
        # Аналогично, простое экранирование
        def simple_escape(text):
             return text.replace('*', '\\*').replace('_', '\\_').replace('`', '\\`').replace('[', '\\[')
        escaped_deadline = simple_escape(deadline)
    ...
    if links_raw:
        ...
        if link_list:
            ...
            for i, link in enumerate(link_list):
                # URL не экранируем, но проверяем, что он выглядит как URL
                # Можно не делать проверку, если доверяем данным
                # Для отображения в Markdown URL внутри () не должен содержать )
                # и не должен содержать незакрытые скобки.
                # Если в URL есть скобки, это может сломать Markdown.
                # Простой способ: если в URL есть скобки, обернуть его в <>
                # Но для Markdown ссылки []() это не сработает.
                # Лучше всего - убедиться, что URL не содержит скобок или они экранированы.
                # Для простоты будем считать, что URL корректен.
                
                # Markdown требует, чтобы URL в () не содержал неэкранированные ) или пробелы.
                # Простой способ экранирования скобок в URL для вставки в Markdown:
                escaped_url = link.replace('(', '\\(').replace(')', '\\)')
                # Это нужно, чтобы не сломать саму ссылку
                
                lines.append(f"  [{i+1}.]({escaped_url})")
        else:
            lines.append("🔗 *Ссылки:* Нет (ошибка формата)")
    # else: # Не добавляем строку, если ссылок нет
